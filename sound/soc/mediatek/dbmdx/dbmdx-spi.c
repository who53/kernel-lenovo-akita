...
static void spi_transport_enable(struct dbmdx_private *p, bool enable)
{
	...
	if(wake_lock_flag){
		...
		wake_lock_flag=0;
		dev_dbg(spi_p->dev, "%s release lock \n", __func__);
	}
#endif
		p->wakeup_release(p);
	}
...
int spi_common_probe(struct spi_device *client)
{
	...
	pinctrl2 = devm_pinctrl_get(&client->dev);
		...
		if (IS_ERR(power_output0)) {
				ret = PTR_ERR(power_output0);
				printk( "%s,Cannot find touch pinctrl state_power_output1!\n",__func__);
				return ret;
		}
		pinctrl_select_state(pinctrl2, power_output1);
		printk( "%s,state_power_output1!\n",__func__);
#endif
