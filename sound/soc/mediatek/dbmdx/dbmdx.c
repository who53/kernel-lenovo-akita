							...
							bool allow_sleep)
{
	...
	if (ret) {
		dev_err(p->dev,	"%s Error resetting TDM lines\n", __func__);
		goto out;
	}
...
static int dbmdx_va_ve_set_asrp_gain(struct dbmdx_private *p, u32 gain_type)
{
	...
	dev_dbg(p->dev,	"%s: Usecase ASRP Gain Type: 0x%x\n", __func__,
					gain_type);
	if ((gain_type & ASRP_TX_OUT_GAIN) && p->pdata->asrp_tx_out_gain > 0) {
		...
		ret = dbmdx_send_cmd(p,	DBMDX_REGN_ASRP_OUTPUT_GAIN_DEST_VALUE |
				(u16)(p->pdata->asrp_tx_out_gain), NULL);
		if (ret < 0) {
			dev_err(p->dev,	"%s: set 0x%x reg error\n", __func__,
	...
	if ((gain_type & ASRP_VCPF_OUT_GAIN) &&
		...
		ret = dbmdx_send_cmd(p,	DBMDX_REGN_ASRP_OUTPUT_GAIN_DEST_VALUE |
				(u16)(p->pdata->asrp_vcpf_out_gain), NULL);
		if (ret < 0) {
			dev_err(p->dev,	"%s: set 0x%x reg error\n", __func__,
					DBMDX_REGN_ASRP_OUTPUT_GAIN_DEST_VALUE);
			return -EIO;
	...
	if ((gain_type & ASRP_RX_OUT_GAIN) && p->pdata->asrp_rx_out_gain > 0) {
		...
		ret = dbmdx_send_cmd(p,	DBMDX_REGN_ASRP_OUTPUT_GAIN_DEST_VALUE |
				(u16)(p->pdata->asrp_rx_out_gain), NULL);
		if (ret < 0) {
			dev_err(p->dev,	"%s: set 0x%x reg error\n", __func__,
					DBMDX_REGN_ASRP_OUTPUT_GAIN_DEST_VALUE);
			return -EIO;
...
static int dbmdx_va_usecase_update(struct dbmdx_private *p,
		struct usecase_config *usecase)
{
	int ret = 0;
	int ret2 = 0;
...
static int dbmdx_usecase_manager(struct dbmdx_private *p, unsigned int cmd)
{
	...
	if (usecase_command_mask & DBMDX_USECASE_MGR_CMD_LOAD_AMODEL) {
		...
		amodel_mode =
			((usecase_detection_mode << AMODEL_DETECTION_MODE_BITS)
					& AMODEL_DETECTION_MODE_MASK);
		amodel_mode |=
			((usecase_model_select_mask << AMODEL_MODEL_SELECT_BITS)
...
#endif
/*wt change for i2s*/
extern int Dbmdx_i2s_Control(int bEnable);
