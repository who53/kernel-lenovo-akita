...
static int get_dsp_dram_sement_by_id(struct audio_dsp_dram *buffer, int id)
{
	...
	if (ret)
		pr_debug("get buffer id = %dis not get\n", id);
...
{
	if (share_buf == NULL) {
		pr_debug("%s msg_atod_share_buf == NULL\n", __func__);
		return -1;
	}
				      ...
				      int id)
{
	...
	if (dsp_mem == NULL) {
		pr_debug("get buffer dsp_mem == NULL\n");
		return -1;
	}
...
int set_task_attr(int dsp_id, int task_enum, int param)
{
	...
	if (!task_attr) {
		pr_debug("%s get task NULL  dsp_id %d\n", __func__, dsp_id);
		return -1;
	}
...
int mtk_adsp_init_gen_pool(struct mtk_base_dsp *dsp)
{
	...
	for (task_id = 0; task_id < AUDIO_TASK_DAI_NUM; task_id++) {
		...
		if (adsp_share_mem == NULL) {
			pr_debug("%s adsp_share_mem = NULL task_id = %d\n",
				__func__,
				task_id);
			continue;
		}
		...
		if (dsp->dsp_mem[task_id].gen_pool_buffer == NULL) {
			pr_debug("%s gen_pool_buffer = NULL\n", __func__);
			continue;
		}
		...
		for (i = 0; i < ADSP_TASK_SHAREMEM_NUM; i++) {
			...
			if (ret < 0) {
				pr_debug("%s not allocate task_id[%d] i[%d]\n",
					__func__, task_id, i);
				continue;
			}
		}
