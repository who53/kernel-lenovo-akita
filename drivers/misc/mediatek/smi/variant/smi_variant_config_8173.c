...
static void mt8173_init_setting(struct mtk_smi_data *smidev,
	bool *default_saved, u32 *default_smi_val,
	unsigned int larbid)
{
	...
	M4U_WriteReg32(SMI_COMMON_EXT_BASE, REG_OFFSET_SMI_L1ARB4,
		default_smi_val[4]);
	M4U_WriteReg32(SMI_COMMON_EXT_BASE, REG_OFFSET_SMI_L1ARB5,
		default_smi_val[5]);
	...
	M4U_WriteReg32(SMI_COMMON_EXT_BASE, 0x234,
		(0x1 << 31) + (0x13 << 26) + (0x14 << 21) +
		(0x0 << 20) + (0x2 << 15) +
		(0x3 << 10) + (0x4 << 5) + 0x5);
	M4U_WriteReg32(SMI_COMMON_EXT_BASE, 0x238,
		(0x2 << 25) + (0x3 << 20) + (0x4 << 15) +
		(0x5 << 10) + (0x6 << 5) + 0x8);
	...
	M4U_WriteReg32(LARB5_BASE, 0x2c,
		M4U_ReadReg32(LARB5_BASE, 0x2c) | (1 << 2));

	/*
	 * confirm. sometimes the reg can not be wrote
	 * while its clock is disable
	 */
...
static void mt8173_vp_setting(struct mtk_smi_data *smidev)
{
	...
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB0, 0x17C0);	/* LARB0, DISP+MDP */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB1, 0x161B);	/* LARB1, VDEC */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB2, 0x1000);	/* LARB2, ISP */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB3, 0x1000);	/* LARB3, VENC+JPG */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB4, 0x17C0);	/* LARB4, DISP2+MDP2 */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB5, 0x1000);	/* LARB5, VENC2 */
...
static void mt8173_vr_setting(struct mtk_smi_data *smidev)
{
	/* VR 4K */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB0, 0x1614);	/* LARB0, DISP+MDP */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB1, 0x1000);	/* LARB1, VDEC */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB2, 0x11F7);	/* LARB2, ISP */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB3, 0x1584);	/* LARB3, VENC+JPG */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB4, 0x1614);	/* LARB4, DISP2+MDP2 */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB5, 0x1584);	/* LARB5, VENC2 */
	...
	M4U_WriteReg32(LARB4_BASE, 0x200, 0x1f);
	/* ovl_ch1_0+ovl_ch1_1 */
...
static void mt8173_hdmi_setting(struct mtk_smi_data *smidev)
{
	/* VP 4K */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB0, 0x1117);	/* LARB0, DISP+MDP */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB1, 0x1659);	/* LARB1, VDEC */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB2, 0x1000);	/* LARB2, ISP */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB3, 0x1000);	/* LARB3, VENC+JPG */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB4, 0x1750);	/* LARB4, DISP2+MDP2 */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB5, 0x1000);	/* LARB5, VENC2 */
...
static void mt8173_hdmi4k_setting(struct mtk_smi_data *smidev)
{
	...
	/* VP 4K */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB0, 0x12A6);	/* LARB0, DISP+MDP */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB1, 0x158B);	/* LARB1, VDEC */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB2, 0x1000);	/* LARB2, ISP */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB3, 0x1000);	/* LARB3, VENC+JPG */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB4, 0x1A6D);	/* LARB4, DISP2+MDP2 */
	M4U_WriteReg32(SMI_COMMON_EXT_BASE,
		REG_OFFSET_SMI_L1ARB5, 0x1000);	/* LARB5, VENC2 */
