...
static int rt5734_read_device(void *client, u32 addr, int len, void *dst)
{
	...
	if (len != 1) {
		pr_debug("%s not support multi read now\n", __func__);
		return -EINVAL;
	}
		...
		uint32_t addr, int len, const void *src)
{
	...
	if (len != 1) {
		pr_debug("%s not support multi write now\n", __func__);
		return -EINVAL;
	}
...
#endif /* CONFIG_RT_REGMAP */
	if (ret < 0)
		pr_debug("%s read addr0x%02x fail\n", __func__, addr);
#endif /* CONFIG_MTK_TINYSYS_SSPM_SUPPORT */
	return ret;
}
...
#endif /* CONFIG_RT_REGMAP */
	if (ret < 0)
		pr_debug("%s write addr0x%02x fail\n", __func__, addr);
#endif /* CONFIG_MTK_TINYSYS_SSPM_SUPPORT */
	return ret;
}
	...
	uint32_t reg, uint32_t mask, uint32_t data)
{
	...
	if (ret < 0) {
		pr_debug("%s fail reg0x%02x data0x%02x\n",
				__func__, reg, data);
		goto OUT_ASSIGN;
	}
...
static int rt5734_check_id(struct spi_device *spi)
{
	...
	if (data != RT5734_CHIPNAME) {
		pr_debug("%s ID(0x%02x) not match\n", __func__, data);
		return -EINVAL;
	}
	return 0;
}
...
static void rt5734_irq_evt_handler(void *info, int eventno)
{
	...
	dev_dbg(chip->dev, "%s eventno = %d\n", __func__, eventno);
	switch (eventno) {
	case RT5734_EVT_OT_SHUTDOWN_RISING:
		pr_debug("%s Enter OT Shutdown\n", __func__);
		break;
	case RT5734_EVT_OT_SHUTDOWN_FALLING:
		pr_debug("%s Exit OT Shutdown\n", __func__);
		break;
	}
}
...
static irqreturn_t rt5734_irq_handler(int irqno, void *param)
{
	...
	if (ret < 0) {
		pr_debug("%s get irq regval fail\n", __func__);
		return IRQ_HANDLED;
	}
	if (regval) {
...
static int rt5734_init_irq(struct rt5734_chip *chip, struct device *dev)
{
	...
	ret = devm_request_threaded_irq(chip->dev, chip->irq,
		...
		"rt5734-irq", chip);
	if (ret < 0) {
		pr_debug("%s request irq fail\n", __func__);
		return -EINVAL;
	}
	/* unmask IRQ */
	ret = rt5734_write_byte(chip->spi, RT5734_IRQ_MASK_R, 0x00);
	if (ret < 0) {
		pr_debug("%s unmask IRQ fail\n", __func__);
		return -EINVAL;
	}
...
static int rt5734_spi_probe(struct spi_device *spi)
{
	...
	if (!chip->regmap_dev) {
		pr_debug("%s register regmap fail\n", __func__);
		return -EINVAL;
	}
