...
int mt6311_read_byte(unsigned char addr, unsigned char *data)
{
	...
	if (ret < 0) {
		pr_debug(MT6311TAG "read Reg[0x%x] fail, ret = %d\n"
			  , addr, ret);
		return ret;
	}
...
int mt6311_write_byte(unsigned char addr, unsigned char data)
{
	...
	mutex_unlock(&mt6311_i2c_access);
	if (ret < 0) {
		pr_debug(MT6311TAG "write Reg[0x%x]=0x%x fail, ret = %d\n"
			  , addr, data, ret);
		return ret;
	}
...
int mt6311_assign_bit(unsigned char reg, unsigned char mask, unsigned char data)
{
	...
	if (ret < 0) {
		pr_debug(MT6311TAG "[%s] read fail(ret=%d), Reg[0x%x] data=0x%x, mask=0x%x\n"
	...
	if (ret < 0) {
		pr_debug(MT6311TAG "[%s] assign bit fail(ret=%d), Reg[0x%x] data=0x%x, mask=0x%x\n"
			  , __func__, ret, reg, data, mask);
		goto OUT_ASSIGN;
	}
OUT_ASSIGN:
	...
	unsigned char mask, unsigned char shift)
{
	...
	tmp = regval;
	if (ret < 0) {
		pr_debug(MT6311TAG "[%s] read fail(ret=%d), Reg[0x%x], mask=0x%x, shift=%d\n"
	...
	unsigned char mask, unsigned char shift)
{
	...
	tmp = regval;
	if (ret < 0) {
		pr_debug(MT6311TAG "[%s] read fail(ret=%d), Reg[0x%x] data=0x%x, mask=0x%x, shift=%d\n"
	...
	ret = mt6311_write_device(reg, 1, &tmp);
	if (ret < 0) {
		pr_debug("[%s] write fail(ret=%d), Reg[0x%x] data=0x%x, mask=0x%x, shift=%d\n"
			  , __func__, ret, reg, data, mask, shift);
		goto OUT_ASSIGN;
	}
OUT_ASSIGN:
...
static int mt6311_check_id(void)
{
	...
	if (ret < 0) {
		/* MT6311 IO fail */
		pr_debug(MT6311TAG "MT6311 check id fail\n");
		return -EIO;
	}

	if (data != MT6311_CID_CODE) {
		/*MT6311 CID(0x%02x) not match */
		pr_debug(MT6311TAG "MT6311 id(0x%x) error\n", data);
		return -EINVAL;
	}
	return 0;
}
...
#ifdef IPIMB_MT6311
	pr_debug(MT6311TAG "regulator not support for SSPM\n");
	return 0;
#else
...
{
#ifdef IPIMB_MT6311
	pr_debug(MT6311TAG "mt6311_access not support for SSPM\n");
	return size;
#else
	int ret;
	...
	pr_debug(MT6311TAG "[%s]\n", __func__);

	if (buf != NULL && size != 0) {
		pr_debug(MT6311TAG "[%s] size is %d, buf is %s\n"
		...
		if (val) {
			...
			pr_debug(MT6311TAG "[%s] write mt6311 reg 0x%x with value 0x%x!\n"
			...
			pr_debug(MT6311TAG "[%s] read mt6311 reg 0x%x with value 0x%x !\n"
				  , __func__, reg_address, g_reg_value_mt6311);
			pr_debug(MT6311TAG "[%s] use \"cat mt6311_access\" to get value\n"
				  , __func__);
		}
	}
	return size;
...
static int __init mt6311_init(void)
{
...
#ifdef IPIMB_MT6311
	pr_debug(MT6311TAG "Kernel driver not support for SSPM\n");
#else
	ret = i2c_add_driver(&mt6311_driver);
	if (ret != 0) {
		pr_debug(MT6311TAG "failed to register mt6311 i2c driver\n");
		return ret;
	}
	...
	if (is_mt6311_exist()) {
		...
		if (ret)
			pr_debug(MT6311TAG "failed to create mt6311_access file(device register fail)\n"
				  );
		ret = platform_driver_register(&mt6311_user_space_driver);
		if (ret)
			pr_debug(MT6311TAG "failed to create mt6311_access file(driver register fail)\n"
				  );
