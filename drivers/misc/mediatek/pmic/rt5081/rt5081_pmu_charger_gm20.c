...
static int _rt5081_set_aicr(struct rt5081_pmu_charger_data *chg_data, u32 uA)
{
	...
	dev_dbg(chg_data->dev, "%s: aicr = %d (0x%02X)\n", __func__, uA,
		reg_aicr);
...
static int rt5081_run_aicl(struct mtk_charger_info *mchr_info, void *data)
{
	...
	if (!(chg_data->irq_flag[RT5081_CHG_IRQIDX_CHGIRQ1] & RT5081_MASK_CHG_MIVR)) {
		dev_dbg(chg_data->dev, "%s: mivr loop is not active\n", __func__);
		return 0;
	}
	dev_dbg(chg_data->dev, "%s: mivr loop is active\n", __func__);
	...
	if (aicl_vth > RT5081_AICL_VTH_MAX) {
		pr_debug("%s: no suitable VTH, vth = %d\n", __func__, aicl_vth);
		ret = -EINVAL;
		goto out;
	}
	...
	*((u32 *)data) = aicr / 100;
	dev_dbg(chg_data->dev, "%s: aicr upper bound = %dmA\n", __func__,
		aicr / 100);
	...
	void *data)
{
	...
	dev_dbg(chg_data->dev, "%s: vbatov = %dmV (0x%02X)\n", __func__, uV / 1000,
		reg_vbatov);
...
static int rt5081_get_vbus(struct mtk_charger_info *mchr_info, void *data)
{
	...
	*((u32 *)data) = adc_vbus /  1000;
	...
	dev_dbg(chg_data->dev, "%s: vbus = %dmV\n", __func__, adc_vbus / 1000);
	return ret;
}
...
static irqreturn_t rt5081_pmu_chg_mivr_irq_handler(int irq, void *data)
{
	...
	if (!mivr_stat) {
		dev_dbg(chg_data->dev, "%s: mivr stat not act\n", __func__);
		goto out;
	}
...
static irqreturn_t rt5081_pmu_ovpctrl_uvp_d_evt_irq_handler(int irq, void *data)
{
	...
	if (!uvp_d_stat) {
		dev_dbg(chg_data->dev, "%s: no uvp_d_stat\n", __func__);
		return IRQ_HANDLED;
	}
