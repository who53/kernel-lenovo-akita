...
{
	/* PMIC MT6355 does not support ISENSE */
	return false;
}
...
{
	unsigned short i;
	unsigned int addr = 0x3200;

	/* All of AUXADC */
	for (i = 0; addr <= 0x337E; i++) {
		adc_dbg_addr[i] = addr;
		addr += 0x2;
	}
	/* Clock related */
	adc_dbg_addr[i++] = MT6355_TOP_CKPDN_CON0;
	adc_dbg_addr[i++] = MT6355_TOP_CKPDN_CON3;
	adc_dbg_addr[i++] = MT6355_TOP_CKHWEN_CON0;
	adc_dbg_addr[i++] = MT6355_TOP_CKHWEN_CON1;
	/* RST related */
	adc_dbg_addr[i++] = MT6355_TOP_RST_CON0;
	/* Others */
	adc_dbg_addr[i++] = MT6355_PCHR_VREF_ANA_DA0;
	adc_dbg_addr[i] = MT6355_BIF_BAT_CON2;
}
...
static void mdrt_reg_dump(void)
{
	...
	pr_debug("AUXADC_ADC16 = 0x%x\n",
		  upmu_get_reg_value(MT6355_AUXADC_ADC16));
	pr_debug("AUXADC_ADC17 = 0x%x\n",
		  upmu_get_reg_value(MT6355_AUXADC_ADC17));
	pr_debug("AUXADC_ADC18 = 0x%x\n",
		  upmu_get_reg_value(MT6355_AUXADC_ADC18));
	pr_debug("AUXADC_ADC36 = 0x%x\n",
	...
	struct device *dev, bool is_timeout, unsigned char ch_num)
{
	...
	dev_notice(dev, "(%d)Time out!STA0=0x%x,STA1=0x%x,STA2=0x%x\n",
		ch_num,
		upmu_get_reg_value(MT6355_AUXADC_STA0),
		upmu_get_reg_value(MT6355_AUXADC_STA1),
		upmu_get_reg_value(MT6355_AUXADC_STA2));
	dev_notice(dev, "RST: Reg[0x%x]=0x%x, Reg[0x%x]=0x%x\n",
		MT6355_STRUP_CON6,
		upmu_get_reg_value(MT6355_STRUP_CON6),
		MT6355_TOP_RST_CON0,
		upmu_get_reg_value(MT6355_TOP_RST_CON0));
}
