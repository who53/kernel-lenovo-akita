					...
					enum dsv_dbg_mode_t mode)
{
	...
	if (irq_count[mode] > g_irq_count_max) {
		...
		pr_debug("%s: DB_VBST = 0x%x, DB_VPOS = 0x%x, DB_VNEG = 0x%x, DBMASK = 0x%x\n",
					...
					enum dsv_dbg_mode_t mode)
{
	...
	if (irq_count[mode] > g_irq_count_max) {
		...
		if (db_vbst < MT6370_DB_VBST_MAX_V) {
			...
			pr_debug("%s: set DB_VBST from 0x%x to 0x%x\n",
...
int mt6370_pmu_dsv_debug_init(struct mt6370_pmu_chip *chip)
{
	...
	if (!mt6370_pmu_dir) {
		pr_debug("create /sys/kernel/debug/mt6370_pmu failed\n");
		return -ENOMEM;
	}
