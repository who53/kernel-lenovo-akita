	...
	enum DLPT_PRIO_TAG prio_val)
{
	...
	pr_debug("[register_dlpt_notify] prio_val=%d\n", prio_val);

	if ((g_dlpt_stop == 0) && (g_dlpt_val != 0)) {
		pr_debug("[register_dlpt_notify] dlpt happen\n");
...
static void exec_dlpt_callback(unsigned int dlpt_val)
{
	...
	if (g_dlpt_stop == 1) {
		pr_debug("[exec_dlpt_callback] g_dlpt_stop=%d\n",
			g_dlpt_stop);
	} else {
...
#else
	pr_debug("[get_dlpt_imix] %d,%d,%d,%d,%d,NA,NA\n",
		volt_avg, curr_avg, g_lbatInt1, ptim_rac_val_avg, imix);
#endif

	if (imix < 0) {
		pr_debug("[get_dlpt_imix] imix = %d < 1\n", imix);
		return g_imix_val;
	}
	return imix;
		...
		const char *buf, size_t size)
{
	...
	pr_debug("[store_dlpt_ut]\n");

	if (buf != NULL && size != 0) {
		pr_debug("[store_dlpt_ut] buf is %s and size is %zu\n",
			buf, size);
		pvalue = (char *)buf;
		ret = kstrtou32(pvalue, 10, (unsigned int *)&val);

		pr_debug("[store_dlpt_ut] your input is %d\n", val);
		...
		const char *buf, size_t size)
{
	...
	pr_debug("[store_dlpt_stop]\n");

	if (buf != NULL && size != 0) {
		pr_debug("[store_dlpt_stop] buf is %s and size is %zu\n"
			, buf, size);
		pvalue = (char *)buf;
		ret = kstrtou32(pvalue, 16, (unsigned int *)&val);
		if ((val != 0) && (val != 1))
			val = 0;
		g_dlpt_stop = val;
		pr_debug("[store_dlpt_stop] g_dlpt_stop=%d\n", g_dlpt_stop);
	}
	return size;
}
