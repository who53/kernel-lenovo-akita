...
#include "gc5035fmipiraw_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc5035f_camera_sensor"
#define LOG_1 LOG_INF("GC5035FMIPI, 2LANE\n")
/****************************   Modify end    *******************************************/
#define GC5035F_DEBUG 1
#if GC5035F_DEBUG
...
static kal_uint32 Dgain_ratio = GC5035F_SENSOR_DGAIN_BASE;
static struct gc5035_otp_t gc5035_otp_data;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5035FMIPI_SENSOR_ID,
	...
	kal_uint8 *data, kal_uint16 length)
{
	...
	if ((((addr & 0x1fff) >> 3) + length) > GC5035F_OTP_DATA_LENGTH) {
...
static void gc5035_gcore_read_dpc(void)
{
	...
	dpcFlag = gc5035_otp_read_byte(GC5035F_OTP_DPC_FLAG_OFFSET);
	LOG_INF("dpc flag = 0x%x\n", dpcFlag);
	switch (GC5035F_OTP_GET_2BIT_FLAG(dpcFlag, 0)) {
	case GC5035F_OTP_FLAG_EMPTY: {
		LOG_INF("dpc info is empty!!\n");
		pDPC->flag = GC5035F_OTP_FLAG_EMPTY;
		break;
	}
	case GC5035F_OTP_FLAG_VALID: {
		LOG_INF("dpc info is valid!\n");
		pDPC->total_num = gc5035_otp_read_byte(GC5035F_OTP_DPC_TOTAL_NUMBER_OFFSET)
			+ gc5035_otp_read_byte(GC5035F_OTP_DPC_ERROR_NUMBER_OFFSET);
		pDPC->flag = GC5035F_OTP_FLAG_VALID;
		LOG_INF("total_num = %d\n", pDPC->total_num);
		break;
	}
	default:
		pDPC->flag = GC5035F_OTP_FLAG_INVALID;
		break;
	}
}
...
static void gc5035_gcore_read_reg(void)
{
	...
	kal_uint8 reg[GC5035F_OTP_REG_DATA_SIZE];
	struct gc5035_reg_update_t *pRegs = &gc5035_otp_data.regs;

	memset(&reg, 0, GC5035F_OTP_REG_DATA_SIZE);
	pRegs->flag = gc5035_otp_read_byte(GC5035F_OTP_REG_FLAG_OFFSET);
	LOG_INF("register update flag = 0x%x\n", pRegs->flag);
	if (pRegs->flag == GC5035F_OTP_FLAG_VALID) {
		gc5035_otp_read_group(GC5035F_OTP_REG_DATA_OFFSET, &reg[0], GC5035F_OTP_REG_DATA_SIZE);

		for (i = 0; i < GC5035F_OTP_REG_MAX_GROUP; i++) {
			base_group = i * GC5035F_OTP_REG_BYTE_PER_GROUP;
			for (j = 0; j < GC5035F_OTP_REG_REG_PER_GROUP; j++)
				if (GC5035F_OTP_CHECK_1BIT_FLAG(reg[base_group], (4 * j + 3))) {
					pRegs->reg[pRegs->cnt].page =
						(reg[base_group] >> (4 * j)) & 0x07;
					pRegs->reg[pRegs->cnt].addr =
						reg[base_group + j * GC5035F_OTP_REG_BYTE_PER_REG + 1];
					pRegs->reg[pRegs->cnt].value =
						reg[base_group + j * GC5035F_OTP_REG_BYTE_PER_REG + 2];
...
#if GC5035F_OTP_FOR_CUSTOMER
unsigned char gc5035_module_id = 0;
unsigned char gc5035_data_awb[6] = {0};
static kal_uint8 gc5035_otp_read_module_info(void)
{
	...
	kal_uint8 flag = 0;
	kal_uint16 check = 0;
	kal_uint16 module_start_offset = GC5035F_OTP_MODULE_DATA_OFFSET;
	kal_uint8 info[GC5035F_OTP_MODULE_DATA_SIZE];
	struct gc5035_module_info_t module_info = { 0 };

	memset(&info, 0, GC5035F_OTP_MODULE_DATA_SIZE);
	memset(&module_info, 0, sizeof(struct gc5035_module_info_t));

	flag = gc5035_otp_read_byte(GC5035F_OTP_MODULE_FLAG_OFFSET);
	LOG_INF("flag = 0x%x\n", flag);

	for (idx = 0; idx < GC5035F_OTP_GROUP_CNT; idx++) {
		switch (GC5035F_OTP_GET_2BIT_FLAG(flag, 2 * (1 - idx))) {
		case GC5035F_OTP_FLAG_EMPTY: {
		...
		case GC5035F_OTP_FLAG_VALID: {
			...
			module_start_offset = GC5035F_OTP_MODULE_DATA_OFFSET
				+ GC5035F_OTP_GET_OFFSET(idx * GC5035F_OTP_MODULE_DATA_SIZE);
			gc5035_otp_read_group(module_start_offset, &info[0], GC5035F_OTP_MODULE_DATA_SIZE);
			for (i = 0; i < GC5035F_OTP_MODULE_DATA_SIZE - 1; i++)
				check += info[i];

			if ((check % 256) == info[GC5035F_OTP_MODULE_DATA_SIZE - 1]) {
                                gc5035_module_id = module_info.module_id;
				module_info.module_id = info[0];
				module_info.lens_id = info[2];
				...
				LOG_INF("check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1, info[GC5035F_OTP_MODULE_DATA_SIZE - 1], (check % 256));
			break;
		}
		case GC5035F_OTP_FLAG_INVALID:
		case GC5035F_OTP_FLAG_INVALID2: {
...
static void gc5035_otp_read_wb_info(void)
{
	...
	kal_uint16 wb_start_offset = GC5035F_OTP_WB_DATA_OFFSET;
	kal_uint16 golden_start_offset = GC5035F_OTP_GOLDEN_DATA_OFFSET;
	kal_uint8 wb[GC5035F_OTP_WB_DATA_SIZE];
	kal_uint8 golden[GC5035F_OTP_GOLDEN_DATA_SIZE];
	struct gc5035_wb_t *pWB = &gc5035_otp_data.wb;
	struct gc5035_wb_t *pGolden = &gc5035_otp_data.golden;

	memset(&wb, 0, GC5035F_OTP_WB_DATA_SIZE);
	memset(&golden, 0, GC5035F_OTP_GOLDEN_DATA_SIZE);
	flag = gc5035_otp_read_byte(GC5035F_OTP_WB_FLAG_OFFSET);
	LOG_INF("flag = 0x%x\n", flag);

	for (idx = 0; idx < GC5035F_OTP_GROUP_CNT; idx++) {
		switch (GC5035F_OTP_GET_2BIT_FLAG(flag, 2 * (3 - idx))) {
		case GC5035F_OTP_FLAG_EMPTY: {
			LOG_INF("wb group %d is empty!\n", idx + 1);
			pWB->flag = pWB->flag | GC5035F_OTP_FLAG_EMPTY;
			break;
		}
		case GC5035F_OTP_FLAG_VALID: {
			LOG_INF("wb group %d is valid!\n", idx + 1);
			wb_start_offset = GC5035F_OTP_WB_DATA_OFFSET
				+ GC5035F_OTP_GET_OFFSET(idx * GC5035F_OTP_WB_DATA_SIZE);
			gc5035_otp_read_group(wb_start_offset, &wb[0], GC5035F_OTP_WB_DATA_SIZE);

			for (i = 0; i < GC5035F_OTP_WB_DATA_SIZE - 1; i++)
				wb_check += wb[i];

			if ((wb_check % 256) == wb[GC5035F_OTP_WB_DATA_SIZE - 1]) {
				pWB->rg = (wb[0] | ((wb[1] & 0xf0) << 4));
				pWB->bg = (((wb[1] & 0x0f) << 8) | wb[2]);
				pWB->rg = pWB->rg == 0 ? GC5035F_OTP_WB_RG_TYPICAL : pWB->rg;
				pWB->bg = pWB->bg == 0 ? GC5035F_OTP_WB_BG_TYPICAL : pWB->bg;
				pWB->flag = pWB->flag | GC5035F_OTP_FLAG_VALID;
				gc5035_data_awb[0] = wb[0];
				gc5035_data_awb[1] = wb[1];
				gc5035_data_awb[2] = wb[2];
				LOG_INF("wb r/g = 0x%x\n", pWB->rg);
				LOG_INF("wb b/g = 0x%x\n", pWB->bg);
			} else {
				pWB->flag = pWB->flag | GC5035F_OTP_FLAG_INVALID;
				LOG_INF("wb check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1, wb[GC5035F_OTP_WB_DATA_SIZE - 1], (wb_check % 256));
			}
			break;
		}
		case GC5035F_OTP_FLAG_INVALID:
		...
		case GC5035F_OTP_FLAG_EMPTY: {
			...
			pGolden->flag = pGolden->flag | GC5035F_OTP_FLAG_EMPTY;
			break;
		}
		case GC5035F_OTP_FLAG_VALID: {
			LOG_INF("golden group %d is valid!\n", idx + 1);
			golden_start_offset = GC5035F_OTP_GOLDEN_DATA_OFFSET
				+ GC5035F_OTP_GET_OFFSET(idx * GC5035F_OTP_GOLDEN_DATA_SIZE);
			gc5035_otp_read_group(golden_start_offset, &golden[0], GC5035F_OTP_GOLDEN_DATA_SIZE);
			for (i = 0; i < GC5035F_OTP_GOLDEN_DATA_SIZE - 1; i++)
				golden_check += golden[i];

			if ((golden_check % 256) == golden[GC5035F_OTP_GOLDEN_DATA_SIZE - 1]) {
				pGolden->rg = (golden[0] | ((golden[1] & 0xf0) << 4));
				pGolden->bg = (((golden[1] & 0x0f) << 8) | golden[2]);
				pGolden->rg = pGolden->rg == 0 ? GC5035F_OTP_WB_RG_TYPICAL : pGolden->rg;
				pGolden->bg = pGolden->bg == 0 ? GC5035F_OTP_WB_BG_TYPICAL : pGolden->bg;
				pGolden->flag = pGolden->flag | GC5035F_OTP_FLAG_VALID;
				gc5035_data_awb[3] = golden[0];
				gc5035_data_awb[4] = golden[1];
				gc5035_data_awb[5] = golden[2];
				LOG_INF("golden r/g = 0x%x\n", pGolden->rg);
				LOG_INF("golden b/g = 0x%x\n", pGolden->bg);
			} else {
				pGolden->flag = pGolden->flag | GC5035F_OTP_FLAG_INVALID;
				LOG_INF("golden check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1, golden[GC5035F_OTP_WB_DATA_SIZE - 1], (golden_check % 256));
			}
			break;
		}
		case GC5035F_OTP_FLAG_INVALID:
...
#if GC5035F_OTP_DEBUG
	kal_uint16 i = 0;
	kal_uint8 debug[GC5035F_OTP_DATA_LENGTH];
#endif
...
#if GC5035F_OTP_DEBUG
	memset(&debug[0], 0, GC5035F_OTP_DATA_LENGTH);
	gc5035_otp_read_group(GC5035F_OTP_START_ADDR, &debug[0], GC5035F_OTP_DATA_LENGTH);
	for (i = 0; i < GC5035F_OTP_DATA_LENGTH; i++)
		pr_debug("addr = 0x%x, data = 0x%x\n", GC5035F_OTP_START_ADDR + i * 8, debug[i]);
#endif
...
static void gc5035_otp_update_dd(void)
{
	...
	if (GC5035F_OTP_FLAG_VALID == pDPC->flag) {
...
{
	kal_uint16 r_gain = GC5035F_OTP_WB_GAIN_BASE;
	kal_uint16 g_gain = GC5035F_OTP_WB_GAIN_BASE;
	kal_uint16 b_gain = GC5035F_OTP_WB_GAIN_BASE;
	kal_uint16 base_gain = GC5035F_OTP_WB_CAL_BASE;
	kal_uint16 r_gain_curr = GC5035F_OTP_WB_CAL_BASE;
	kal_uint16 g_gain_curr = GC5035F_OTP_WB_CAL_BASE;
	kal_uint16 b_gain_curr = GC5035F_OTP_WB_CAL_BASE;
	kal_uint16 rg_typical = GC5035F_OTP_WB_RG_TYPICAL;
	kal_uint16 bg_typical = GC5035F_OTP_WB_BG_TYPICAL;
	struct gc5035_wb_t *pWB = &gc5035_otp_data.wb;
	struct gc5035_wb_t *pGolden = &gc5035_otp_data.golden;

	if (GC5035F_OTP_CHECK_1BIT_FLAG(pGolden->flag, 0)) {
		rg_typical = pGolden->rg;
		bg_typical = pGolden->bg;
	} else {
		rg_typical = GC5035F_OTP_WB_RG_TYPICAL;
		bg_typical = GC5035F_OTP_WB_BG_TYPICAL;
	}
	LOG_INF("typical rg = 0x%x, bg = 0x%x\n", rg_typical, bg_typical);

	if (GC5035F_OTP_CHECK_1BIT_FLAG(pWB->flag, 0)) {
		r_gain_curr = GC5035F_OTP_WB_CAL_BASE * rg_typical / pWB->rg;
		b_gain_curr = GC5035F_OTP_WB_CAL_BASE * bg_typical / pWB->bg;
		g_gain_curr = GC5035F_OTP_WB_CAL_BASE;

		base_gain = (r_gain_curr < b_gain_curr) ? r_gain_curr : b_gain_curr;
		base_gain = (base_gain < g_gain_curr) ? base_gain : g_gain_curr;

		r_gain = GC5035F_OTP_WB_GAIN_BASE * r_gain_curr / base_gain;
		g_gain = GC5035F_OTP_WB_GAIN_BASE * g_gain_curr / base_gain;
		b_gain = GC5035F_OTP_WB_GAIN_BASE * b_gain_curr / base_gain;
...
static void gc5035_otp_update_reg(void)
{
	...
	if (GC5035F_OTP_CHECK_1BIT_FLAG(gc5035_otp_data.regs.flag, 0))
...
static kal_uint8 gc5035_otp_identify(void)
{
	...
	gc5035_otp_read_group(GC5035F_OTP_ID_DATA_OFFSET, &gc5035_otp_data.otp_id[0], GC5035F_OTP_ID_SIZE);
...
static void gc5035_otp_function(void)
{
	...
	kal_uint8 otp_id[GC5035F_OTP_ID_SIZE];

	memset(&otp_id, 0, GC5035F_OTP_ID_SIZE);
	...
	gc5035_otp_read_group(GC5035F_OTP_ID_DATA_OFFSET, &otp_id[0], GC5035F_OTP_ID_SIZE);
	for (i = 0; i < GC5035F_OTP_ID_SIZE; i++)
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < GC5035F_SENSOR_GAIN_BASE)
		reg_gain = GC5035F_SENSOR_GAIN_BASE;
	else if (reg_gain > GC5035F_SENSOR_GAIN_MAX)
		reg_gain = GC5035F_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 GC5035F_AGC_Param[GC5035F_SENSOR_GAIN_MAP_SIZE][2] = {
		{  256,  0 },
		{  302,  1 },
		{  358,  2 },
		{  425,  3 },
	...
	for (gain_index = GC5035F_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= GC5035F_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xb6, GC5035F_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * Dgain_ratio / GC5035F_AGC_Param[gain_index][0];
	write_cmos_sensor(0xb1, (temp_gain >> 8) & 0x0f);
	write_cmos_sensor(0xb2, temp_gain & 0xfc);
	LOG_INF("Exit! GC5035F_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d\n",
		GC5035F_AGC_Param[gain_index][1], temp_gain, reg_gain);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC5035F_MIRROR);
	...
	write_cmos_sensor(0x54, GC5035F_RSTDUMMY1);
	...
	write_cmos_sensor(0x22, GC5035F_RSTDUMMY2);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_INF("gc5035_get_imgsensor_id:\n");
	...
	LOG_INF("gc5035_get_imgsensor_id: %d\n", *sensor_id);
...
UINT32 GC5035FMIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	LOG_INF("UINT32 GC5035FMIPI_RAW_SensorInit");	    
