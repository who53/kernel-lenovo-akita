...
unsigned char gc5035_data_af[GC5035_OTP_AF_DATA_SIZE]={0};
...
static void gc5035_otp_read_af_info(void)
{
	...
	kal_uint8 info[GC5035_OTP_AF_DATA_SIZE];
	struct gc5035_af_info_t af_info = { 0 };

	memset(&info, 0, GC5035_OTP_AF_DATA_SIZE);
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			module_start_offset = GC5035_OTP_AF_DATA_OFFSET
				+ GC5035_OTP_GET_OFFSET(idx * GC5035_OTP_AF_DATA_SIZE);
			gc5035_otp_read_group(module_start_offset, &info[0], GC5035_OTP_AF_DATA_SIZE);
			for (i = 0; i < GC5035_OTP_AF_DATA_SIZE - 1; i++)
				check += info[i];

			if ((check % 255 + 1) == info[GC5035_OTP_AF_DATA_SIZE - 1]) {
				//af_info.af_dir = info[0];
				af_info.macro_low= info[0];
				af_info.macro_high = info[1];
				af_info.inf_low = info[2];
				af_info.inf_high = info[3];
				for(i = 0; i < GC5035_OTP_AF_DATA_SIZE; i++){
					gc5035_data_af[i] = info[i];
				}
				...
				LOG_INF("check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1, info[GC5035_OTP_AF_DATA_SIZE - 1], (check % 255 + 1));
			break;
		}
		case GC5035_OTP_FLAG_INVALID:
		case GC5035_OTP_FLAG_INVALID2: {
...
{
	LOG_INF("UINT32 GC5035MIPI_RAW_SensorInit");	    
