...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5035BMIPI_SENSOR_ID,
...
#if GC5035B_OTP_FOR_CUSTOMER
unsigned char gc5035b_data_af[GC5035B_OTP_AF_DATA_SIZE-1]={0};
unsigned char gc5035b_module_id = 0;
unsigned char gc5035b_data_awb[6] = {0};
static kal_uint8 gc5035_otp_read_module_info(void)
{
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			if ((check % 256) == info[GC5035_OTP_MODULE_DATA_SIZE - 1]) {
				module_info.module_id = info[0];
				gc5035b_module_id = module_info.module_id;
				module_info.camera_type= info[1];
				module_info.lens_id = info[2];
				module_info.vcm_id = info[3];
				...
				LOG_INF("check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1, info[GC5035_OTP_MODULE_DATA_SIZE - 1], (check % 256));
			break;
		}
...
static void gc5035_otp_read_wb_info(void)
{
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			if ((wb_check % 256) == wb[GC5035_OTP_WB_DATA_SIZE - 1]) {
				...
				gc5035b_data_awb[0] = wb[0];
				gc5035b_data_awb[1] = wb[1];
				gc5035b_data_awb[2] = wb[2];
				LOG_INF("wb r/g = 0x%x\n", pWB->rg);
				LOG_INF("wb b/g = 0x%x\n", pWB->bg);
			} else {
				pWB->flag = pWB->flag | GC5035_OTP_FLAG_INVALID;
				LOG_INF("wb check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1, wb[GC5035_OTP_WB_DATA_SIZE - 1], (wb_check % 256));
			}
			break;
		}
		case GC5035_OTP_FLAG_INVALID:
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			if ((golden_check % 256) == golden[GC5035_OTP_GOLDEN_DATA_SIZE - 1]) {
				...
				gc5035b_data_awb[3] = golden[0];
				gc5035b_data_awb[4] = golden[1];
				gc5035b_data_awb[5] = golden[2];
				...
				LOG_INF("golden check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1, golden[GC5035_OTP_WB_DATA_SIZE - 1], (golden_check % 256));
			}
			break;
		}
		case GC5035_OTP_FLAG_INVALID:
...
static void gc5035_otp_read_af_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 idx = 0;
	kal_uint8 flag = 0;
	kal_uint16 check = 0;
	kal_uint16 module_start_offset = GC5035_OTP_AF_DATA_OFFSET;
	kal_uint8 info[GC5035B_OTP_AF_DATA_SIZE];
	struct gc5035_af_info_t af_info = { 0 };

	memset(&info, 0, GC5035B_OTP_AF_DATA_SIZE);
	memset(&af_info, 0, sizeof(struct gc5035_af_info_t));

	flag = gc5035_otp_read_byte(GC5035_OTP_AF_FLAG_OFFSET);
	LOG_INF("flag = 0x%x\n", flag);
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
			LOG_INF("group %d is valid!\n", idx + 1);
			module_start_offset = GC5035_OTP_AF_DATA_OFFSET
				+ GC5035_OTP_GET_OFFSET(idx * GC5035B_OTP_AF_DATA_SIZE);
			gc5035_otp_read_group(module_start_offset, &info[0], GC5035B_OTP_AF_DATA_SIZE);
			for (i = 0; i < GC5035B_OTP_AF_DATA_SIZE - 1; i++)
				check += info[i];

			if ((check % 256) == info[GC5035B_OTP_AF_DATA_SIZE - 1]) {
				af_info.af_dir = info[0];
				af_info.macro_low= info[1];
				af_info.macro_high = info[2];
				af_info.inf_low = info[3];
				af_info.inf_high = info[4];
				for(i = 1; i < GC5035B_OTP_AF_DATA_SIZE; i++){
					gc5035b_data_af[i-1] = info[i];
				}

				LOG_INF("af_dir = 0x%x\n", af_info.af_dir);
				LOG_INF("macro = 0x%x\n", (af_info.macro_high<<8|af_info.macro_low));
				LOG_INF("inf = 0x%x\n", (af_info.inf_high<<8|af_info.inf_low));
			} else
				LOG_INF("check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1, info[GC5035B_OTP_AF_DATA_SIZE - 1], (check % 256));
			break;
		}
		case GC5035_OTP_FLAG_INVALID:
		case GC5035_OTP_FLAG_INVALID2: {
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				module_info = gc5035_otp_identify();

				if(module_info != 0x0a){
					*sensor_id = 0xFFFFFFFF;
					pr_err("GC5035 module_info err 0x%x",module_info);
					return ERROR_SENSOR_CONNECT_FAIL;
				}

				LOG_INF("[gc5035_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
...
UINT32 GC5035BMIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	LOG_INF("UINT32 GC5035BMIPI_RAW_SensorInit");	    
