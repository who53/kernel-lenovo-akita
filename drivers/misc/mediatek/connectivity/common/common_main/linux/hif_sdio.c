							...
							UINT16 func_num)
{
	...
	if (i == CFG_CLIENT_COUNT) {
		/*
		 * pr_debug(DRV_NAME "Cannot find vendor:0x%x, device:0x%x, func_num:0x%x, i=%d\n",
...
stp_off_exist:
	...
	sdio_release_host(func);
	if (ret)
		pr_debug(DRV_NAME "sdio_release_irq for stp fail(%d)\n", ret);
	else
		pr_debug(DRV_NAME "sdio_release_irq for stp ok\n");
#endif
	clt_index = g_hif_sdio_probed_func_list[probe_index].clt_idx;
	if (clt_index >= 0) {	/* the function has been registered */
		func = g_hif_sdio_probed_func_list[probe_index].func;
...
wifi_off_exist:
	...
	sdio_release_host(func);
	if (ret)
		pr_debug(DRV_NAME "sdio_release_irq for wifi fail(%d)\n", ret);
	else
		pr_debug(DRV_NAME "sdio_release_irq for wifi ok\n");
