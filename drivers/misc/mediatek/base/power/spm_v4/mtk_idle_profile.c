	...
	if (type < IDLE_TYPE_RG) {
		...
		if (interval >= IDLE_RATIO_WINDOW_MS) {
			...
			ratio->value = div_s64(
					...
					last_ratio, IDLE_RATIO_WINDOW_MS)
					+ last_idle_time;
			ratio->value_dp = div_s64(
					(IDLE_RATIO_WINDOW_MS - interval) *
					last_ratio_dp, IDLE_RATIO_WINDOW_MS)
					+ ((type == IDLE_TYPE_DP)
							? last_idle_time : 0);
			ratio->value_so3 = div_s64(
					(IDLE_RATIO_WINDOW_MS - interval) *
					last_ratio_so3, IDLE_RATIO_WINDOW_MS)
					+ ((type == IDLE_TYPE_SO3)
							? last_idle_time : 0);
			ratio->value_so = div_s64(
					(IDLE_RATIO_WINDOW_MS - interval) *
					last_ratio_so, IDLE_RATIO_WINDOW_MS)
					+ ((type == IDLE_TYPE_SO)
							? last_idle_time : 0);
...
#if 0
		pr_err("XXIDLE %llu, %llu, %llu, %llu, %llu, %llu, %llu, %llu, %d\n",
			ratio->last_end_ts, ratio->start_ts, ratio->end_ts,
			ratio->value, ratio->value_dp,
			ratio->value_so3, ratio->value_so, last_ratio, type);
#endif
...
bool mtk_idle_select_state(int type, int reason)
{
	...
	dump_block_info	= ((curr_time - p_idle->prev_time)
			> p_idle->time_critera)
		...
		unsigned long *block_cnt, unsigned int *block_mask)
{
	...
	if (cnt && block_cnt && block_mask)
		...
		pr_err("IDLE BLOCKING INFO SETTING FAIL (type:%d)\n", type);
}
...
void dpidle_show_profile_time(void)
{
	...
	if (dpidle_profile_cnt > 0) {
		...
		pr_crit("%s", get_idle_buf(latency_profile_log));
	}
}
