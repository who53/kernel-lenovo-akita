...
int spm_to_sspm_command_async(u32 cmd, struct spm_data *spm_d)
{
	...
	case SPM_LEAVE_SODI3:
		...
		if (ret != 0)
			pr_debug("#@# %s(%d) sspm_ipi_send_async(cmd:0x%x) ret %d\n",
				__func__, __LINE__, cmd, ret);
		break;
	default:
		pr_debug("#@# %s(%d) cmd(%d) wrong!!!\n", __func__,
			__LINE__, cmd);
		break;
	}
...
int spm_to_sspm_command_async_wait(u32 cmd)
{
	...
	case SPM_LEAVE_SODI3:
		...
		if (ret != 0) {
			pr_debug("#@# %s(%d) sspm_ipi_send_async_wait(cmd:0x%x) ret %d\n",
				__func__, __LINE__, cmd, ret);
		} else if (ack_data < 0) {
			ret = ack_data;
