...
static void clk_buf_ctrl_internal(enum clk_buf_id id, bool onoff)
{
	...
	case CLK_BUF_CONN:
		...
		pr_debug("%s: id=%d, onoff=%d, DCXO_ENABLE=0x%x, pwrap_dcxo_en_flag=0x%x\n",
...
bool clk_buf_ctrl(enum clk_buf_id id, bool onoff)
{
	...
	case CLK_BUF_CONN:
		...
		if (!(pwrap_dcxo_en_flag & DCXO_CONN_ENABLE)) {
			ret = -1;
			pr_debug("%s: id=%d skip due to non co-clock for CONN\n",
				__func__, id);
			pmic_clk_buf_ctrl_wcn(0);
			pmic_clk_buf_swctrl[XO_WCN] = 0;
			break;
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (!kstrtoint(buf, 10, &debug)) {
		if (debug == 0)
			...
			pr_debug("bad argument!! should be 0 or 1[0: disable, 1: enable]\n");
	} else
		return -EPERM;
