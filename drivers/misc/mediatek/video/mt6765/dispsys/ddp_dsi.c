	...
	void *cmdq_trigger_handle, enum CMDQ_STATE state)
{
	...
	struct LCM_DSI_PARAMS *dsi_params = NULL;
        /**[QUPGRADE-5][M8 FHD]add for hx8279d esd check begin**/
	//struct DSI_T0_INS t0, t1;
	struct DSI_T0_INS t0, t1, t2;
        /**[QUPGRADE-5][M8 FHD]add for hx8279d esd check end**/
	struct DSI_RX_DATA_REG read_data0;
	struct DSI_RX_DATA_REG read_data1;
	struct DSI_RX_DATA_REG read_data2;
	struct DSI_RX_DATA_REG read_data3;
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < 3; i++) {
			...
			/**[QUPGRADE-5][M8 FHD]add for hx8279d esd check begin**/
			if(dsi_params->lcm_esd_check_table[i].page_count){
				t2.CONFG = 0x00;
				t2.Data_ID = 0x15;
				t2.Data0 = dsi_params->lcm_esd_check_table[i].page_cmd;
				t2.Data1 = dsi_params->lcm_esd_check_table[i].page_para_list[0];

				//printk("page_cmd: %x, page_para_list: %x\n", dsi_params->lcm_esd_check_table[i].page_cmd,
				//dsi_params->lcm_esd_check_table[i].page_para_list[0]);

				/* write DSI CMDQ */
				DSI_OUTREG32(cmdq_trigger_handle,
					&DSI_CMDQ_REG[dsi_i]->data[0], AS_UINT32(&t2));
				DSI_OUTREG32(cmdq_trigger_handle,
					&DSI_CMDQ_REG[dsi_i]->data[1], AS_UINT32(&t1));
				DSI_OUTREG32(cmdq_trigger_handle,
					&DSI_CMDQ_REG[dsi_i]->data[2], AS_UINT32(&t0));
				DSI_OUTREG32(cmdq_trigger_handle,
					&DSI_REG[dsi_i]->DSI_CMDQ_SIZE, 3);
			}else{
        		/**[QUPGRADE-5][M8 FHD]add for hx8279d esd check end**/
				DSI_OUTREG32(cmdq_trigger_handle,
