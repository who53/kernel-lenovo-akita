...
extern unsigned char hi556_data_af[MODULE_AF_DATA_SIZE + 1];
...
extern unsigned char s5k4h7_otp_data[S5K4H7_OTP_SIZE];

#define MODULE_INFO_START 0x0B
//AF
#define AF_INFO_START 0x27
#define AF_DATA_SIZE 5
extern unsigned char s5k4h7_af_data_valid;

//AWB
#define AWB_INFO_START 0x33
#define AWB_DATA_SIZE 0x1D
extern unsigned char s5k4h7_awb_data_valid;

//LSC
#define LSC_INFO_START 0x0052
#define LSC_DATA_SIZE 1868
extern unsigned char s5k4h7_lsc_data_valid;
#define LSC_DATA_SIZE 1868
#endif
...
#define AF_INFO_START 0x27
#define AF_DATA_SIZE 5
extern unsigned char s5k4h7_af_data_valid1;

//AWB
#define AWB_INFO_START 0x33
#define AWB_DATA_SIZE 0x1D
extern unsigned char s5k4h7_awb_data_valid1;

//LSC
#define LSC_INFO_START 0x0052
#define LSC_DATA_SIZE 1868
extern unsigned char s5k4h7_lsc_data_valid1;
#define LSC_DATA_SIZE 1868
#endif
...
#endif
{
    ...
    if (ptempbuf->sensorID == 0x556 && ptempbuf->u4Length == 0x05 && ptempbuf->u4Offset == 0x1a37 && hi556_af_valid){//HI556 AF data
      ...
      i4RetValue = MODULE_AF_DATA_SIZE;
      kfree(pBuff);
      kfree(pu1Params);
      return i4RetValue;
    }
#endif
#if GC5035B_OTP_FUNCTION
    if (ptempbuf->sensorID == 0x5036 && ptempbuf->u4Length == 0x1 && ptempbuf->u4Offset == 0x1e88){//HI846 MID data
      if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)&gc5035b_module_id, ptempbuf->u4Length)) {
        kfree(pBuff);
        kfree(pu1Params);
        pr_debug("ioctl copy to user failed\n");
        return -EFAULT;
    ...
    if (ptempbuf->sensorID == 0x5036 && ptempbuf->u4Length == 0x06 && ptempbuf->u4Offset == 0x1f18){
      if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &gc5035b_data_awb[0], ptempbuf->u4Length)) {
        kfree(pBuff);
        kfree(pu1Params);
        pr_debug("ioctl copy to user failed\n");
        return -EFAULT;
    ...
    if (ptempbuf->sensorID == 0x5036 && ptempbuf->u4Length == 0x05 && ptempbuf->u4Offset == 0x1f98){
      if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &gc5035b_data_af[0], ptempbuf->u4Length)) {
        kfree(pBuff);
        kfree(pu1Params);
        pr_debug("ioctl copy to user failed\n");
        return -EFAULT;
...
#endif
#if GC5035_OTP_FUNCTION
    if (ptempbuf->sensorID == 0x5035 && ptempbuf->u4Length == 0x1 && ptempbuf->u4Offset == 0x1ef8){//HI846 MID data
      if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)&gc5035_module_id, ptempbuf->u4Length)) {
        kfree(pBuff);
        kfree(pu1Params);
        pr_debug("ioctl copy to user failed\n");
        return -EFAULT;
    ...
    if (ptempbuf->sensorID == 0x5035 && ptempbuf->u4Length == 0x06 && ptempbuf->u4Offset == 0x1f80){
      if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &gc5035_data_awb[0], ptempbuf->u4Length)) {
        kfree(pBuff);
        kfree(pu1Params);
        pr_debug("ioctl copy to user failed\n");
        return -EFAULT;
    ...
    if (ptempbuf->sensorID == 0x5035 && ptempbuf->u4Length == 0x05 && ptempbuf->u4Offset == 0x1ea0){
      if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &gc5035_data_af[0], ptempbuf->u4Length)) {
        kfree(pBuff);
        kfree(pu1Params);
        pr_debug("ioctl copy to user failed\n");
        return -EFAULT;
      ...
      return i4RetValue;
...
#if S5K4H7_OTP
		if (ptempbuf->sensorID == 0x487B && ptempbuf->u4Length == 0x1 && ptempbuf->u4Offset == 0x0B){
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)&s5k4h7_otp_data[MODULE_INFO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		...
		if (ptempbuf->sensorID == 0x487B && ptempbuf->u4Length == 0x05 && ptempbuf->u4Offset == 0x27 && s5k4h7_af_data_valid){
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &s5k4h7_otp_data[AF_INFO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		  }
			i4RetValue = AF_DATA_SIZE;
			kfree(pBuff);
			kfree(pu1Params);
			return i4RetValue;
		}
		if (ptempbuf->sensorID == 0x487B && ptempbuf->u4Length == 0x1D  && ptempbuf->u4Offset == 0x33 && s5k4h7_awb_data_valid){
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &s5k4h7_otp_data[AWB_INFO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		...
		if (ptempbuf->sensorID == 0x487B && ptempbuf->u4Length == 0x74C && ptempbuf->u4Offset == LSC_INFO_START && s5k4h7_lsc_data_valid){
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &s5k4h7_otp_data[LSC_INFO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
...
//s5k4h7_qtech
#if S5K4H7QTECH_OTP
		if (ptempbuf->sensorID == (0x487B + 1) && ptempbuf->u4Length == 0x1 && ptempbuf->u4Offset == 0x0B){
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)&s5k4h7_otp_data1[MODULE_INFO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		...
		if (ptempbuf->sensorID == (0x487B + 1)&& ptempbuf->u4Length == 0x05 && ptempbuf->u4Offset == 0x27 && s5k4h7_af_data_valid){
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &s5k4h7_otp_data1[AF_INFO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		...
		if (ptempbuf->sensorID == (0x487B + 1) && ptempbuf->u4Length == 0x1D  && ptempbuf->u4Offset == 0x33 && s5k4h7_awb_data_valid){
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &s5k4h7_otp_data1[AWB_INFO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
		...
		if (ptempbuf->sensorID == (0x487B + 1) && ptempbuf->u4Length == 0x74C && ptempbuf->u4Offset == LSC_INFO_START && s5k4h7_lsc_data_valid){
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &s5k4h7_otp_data1[LSC_INFO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_debug("ioctl copy to user failed\n");
				return -EFAULT;
