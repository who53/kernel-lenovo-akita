...
bool tlc_notify_event(u32 event_type)
{
	...
	if (!dci) {
		pr_debug("%s: DCI has not been set up properly - exiting\n",
			__func__);
		return false;
	}
...
static int start_thread_if_needed(void)
{
	...
	if (IS_ERR_OR_NULL(thread_id)) {
		...
		pr_debug("Unable to start Trusted UI main thread: %d\n", rc);
		thread_id = NULL;
	}
