			...
			const char __user *buf, size_t size, loff_t *ppos)
{
	...
	pr_debug("test_case = %d, test_cpu = %d", test_case, test_cpu);
	if ((msg[1] != ':') || (test_case < 1) || (test_case > 6)
	    || (test_cpu < 0) || (test_cpu > nr_cpu_ids)) {
		pr_debug("WDT test - Usage: [test case number(1~6):test cpu(0~%d)]\n"
								, nr_cpu_ids);
		return -EINVAL;
	}

	if (test_case == 1) {
		pr_debug("Test 1 : One CPU WDT timeout (smp_send_stop succeed)\n");
	} else if (test_case == 2) {
		pr_debug("Test 2 : One CPU WDT timeout, other CPU disable irq (smp_send_stop fail in old design)\n");
	} else if (test_case == 3) {
		pr_debug("Test 3 : WDT timeout and loop in panic flow\n");
	} else if (test_case == 4) {
		pr_debug("Test 4 : All CPU WDT timeout (other CPU stop in the loop)\n");
	} else if (test_case == 5) {
		pr_debug("Test 5 : Disable ALL CPU IRQ/FIQ (FIQ : HW_reboot, ATF : HWT\n");
	} else if (test_case == 6) {
		pr_debug("Test 6 : (For ATF) HW_REBOOT : change SMC call back function and while loop\n");
#ifdef CONFIG_ARM64
	...
	for (i = 0; i < nr_cpu_ids; i++) {
		pr_debug(" wake_up_process(wk_tsk[%d])\n", i);
...
static int handler_fault(struct kprobe *p, struct pt_regs *regs, int trapnr)
{
	pr_debug("fault_handler: p->addr = 0x%p, trap #%dn", p->addr, trapnr);
				...
				char __user *buf, size_t size, loff_t *ppos)
{
	...
	if (copy_to_user(buf, buffer, len))
		pr_debug("%s fail to output info.\n", __func__);
						 ...
						 loff_t *ppos)
{
	...
	if ((size < 5) || (size >= sizeof(msg))) {
		pr_debug("aed: %s size sould be >= 5 and <= %zx bytes.\n",
				__func__, sizeof(msg));
		return -EINVAL;
	}

	if (copy_from_user(msg, buf, size)) {
		pr_debug("aed: %s unable to read message\n", __func__);
		return -EFAULT;
	}
	/* Be safe */
	msg[size] = 0;
	...
	if ((colon_ptr == NULL) || ((colon_ptr - msg) > 32)) {
		pr_debug("aed: %s cannot find valid module name\n", __func__);
		return -EINVAL;
	}
	*colon_ptr = 0;
