		...
		size_t cnt, loff_t *data)
{
	...
	if (val == 1) {
		...
		hang_aee_warn = 0;
		pr_debug("[hang_detect] disable coredump.\n");
	} else if (val == 5) {
		hang_aee_warn = 2;
		pr_debug("[hang_detect] denable coredump.\n");
	} else if (val > 10) {
		...
		unsigned long arg)
{
	...
	if ((cmd == AEEIOCTL_SET_HANG_FLAG) &&
		...
		if ((int)arg == 1) {
			hang_aee_warn = 2;
			pr_debug("hang_detect: aee enable system_server coredump.\n");
		}
		return ret;
	}
...
static int __init monitor_hang_init(void)
{
	...
	if (unlikely(err)) {
		pr_debug("aee: failed to register aed_wdt_RT_Monitor_dev device!\n");
		return err;
	}
	hang_detect_init();
	/* bleow code is added by QHQ  for hang detect */
...
static int hang_detect_warn_thread(void *arg)
{
	...
	pr_debug("hang_detect create warning api: %s.", string_tmp);
#ifdef __aarch64__
...
static int hang_detect_dump_thread(void *arg)
{
	...
	pr_debug("[Hang_Detect] hang_detect dump thread exit.\n");
	return 0;
}
