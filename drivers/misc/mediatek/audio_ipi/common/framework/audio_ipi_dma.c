...
int init_audio_ipi_dma(const uint32_t dsp_id)
{
	...
	if (dma == NULL) {
		pr_debug("scp_get_reserve_mem_virt(%d) fail!!", mem_id);
		ret = -ENOMEM;
		goto IPI_DMA_INIT_EXIT;
	}
	...
	if (dma->base_phy.addr_val == 0) {
		pr_debug("get_reserve_mem_phys(%d) fail!!", mem_id);
		ret = -ENOMEM;
		goto IPI_DMA_INIT_EXIT;
	}
	...
	if (gen_pool_add_virt(
		    ...
		    -1) != 0) {
		pr_debug("dsp_id(%u) gen_pool_add fail", dsp_id);
		ret = -ENOMEM;
	...
	const uint32_t size)
{
	...
	if (new_addr == 0) {
		pr_debug("task %d, gen_pool_alloc(0x%x) fail, (%zu/%zu)",
			  task,
			  size,
	...
	*phy_addr = gen_pool_virt_to_phys(g_dma_pool[dsp_id], new_addr);
	if (*phy_addr == 0) {
		pr_debug("task %d, gen_pool_virt_to_phys() fail, g_dma_pool[%u] %p, new_addr %zu",
			  task,
			  dsp_id,
			  g_dma_pool[dsp_id],
			  new_addr);
			       ...
			       const uint32_t dsp_to_ap_size)
{
	...
	if (g_region_reg_flag[task] == true) {
		pr_debug("task: %d already register", task);
		return -EEXIST;
	}
...
static uint32_t audio_region_data_count(struct audio_region_t *region)
{
	...
	if (region->read_idx >= region->size) {
		DUMP_REGION(pr_debug, "read_idx fail", region, count);
		region->read_idx %= region->size;
	} else if (region->write_idx >= region->size) {
		DUMP_REGION(pr_debug, "write_idx fail", region, count);
		...
		uint32_t count)
{
	...
	if (region->size == 0) {
		DUMP_REGION(pr_debug, "size fail", region, count);
		return -ENODEV;
	}
	...
	if (free_space < count_align) {
		DUMP_REGION(pr_debug, "free_space < count_align", region,
			    count);
		return -EOVERFLOW;
	}
				       ...
				       uint32_t count)
{
	...
	if (count_align > available_count) {
		DUMP_REGION(pr_debug, "count_align > available_count",
			    region, count);
		return -ENOMEM;
	}
...

{
	...
	if (drop_size == 0) {
		pr_debug("task: %d, drop_size = 0", task);
		return -ENODATA;
	}
...
static void hal_dma_dump_msg_in_queue(struct hal_dma_queue_t *msg_queue)
{
	...
	pr_debug("idx_r: %u, idx_w: %u, queue(%u/%u)",
		msg_queue->idx_r,
		msg_queue->idx_w,
	...
	uint32_t *p_idx_msg)
{
	...
	if (msg_queue == NULL || p_ipi_msg == NULL || p_idx_msg == NULL) {
		pr_debug("NULL!! msg_queue: %p, p_ipi_msg: %p, p_idx_msg: %p",
	...
	if (hal_dma_check_queue_to_be_full(msg_queue) == true) {
		pr_debug("task: %d, msg_id: 0x%x, queue overflow, idx_r: %u, idx_w: %u, drop it",
	...
	if (hal_dma_check_idx_msg_valid(msg_queue, msg_queue->idx_w) == false) {
		pr_debug("idx_w %u is invalid!! return", msg_queue->idx_w);
		return -1;
	}

	/* get dma data */
	if (p_ipi_msg->dma_info.data_size > MAX_DSP_DMA_WRITE_SIZE) {
		pr_debug("data_size %u > %u!!",
	...
	for (i = 0; i < p_ipi_msg->dma_info.data_size; i++)
		pr_debug("%d", msg_queue->tmp_buf_d2k[i]);
#endif
...
int audio_ipi_dma_msg_to_hal(struct ipi_msg_t *p_ipi_msg)
{
	...
	if (p_ipi_msg == NULL || msg_queue == NULL) {
		pr_debug("p_ipi_msg(%p) or msg_queue(%p) is NULL!! return",
			p_ipi_msg, msg_queue);
		return -EFAULT;
	}
