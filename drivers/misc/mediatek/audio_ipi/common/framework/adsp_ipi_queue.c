...
#define IPI_ASSERT(exp) \
	do { \
		if (!(exp)) { \
			pr_debug("ASSERT("#exp")!! \""  __FILE__ "\", %uL", \
				  __LINE__); \
		} \
	} while (0)
#endif
	...
	uint32_t wait_ms)
{
	...
	for (try_cnt = 0; try_cnt < k_max_try_cnt; try_cnt++) {
		...
		if (retval > 0) {
			pr_debug("wait ret %d, sig %d, enable %d",
				  retval,
		...
		if (retval == 0) { /* timeout */
			pr_debug("wait %u ms timeout, will still send to scp later!!",
				wait_ms);
			break;
		}
		...
		pr_debug("retval: %d not handle!!", retval);
	}
	...
	void (*ipi_handler)(int ipi_id, void *buf, unsigned int len))
{
	...
	if (ipi_handler == NULL) {
		pr_debug("NULL!! ipi_handler: %p", ipi_handler);
		return -EFAULT;
	}
	...
	if (stop_time > 1000) { /* 1 ms */
		pr_debug("IPI Q push %lld us too long!! push %lld wake %lld, ktime api %lld",
			  stop_time,
			  push_time,
			  wake_time - push_time,
			  stop_time - wake_time);
...
static void scp_dump_msg_in_queue(struct scp_msg_queue_t *msg_queue)
{
	...
	pr_debug("dsp_id: %u, idx_r: %u, idx_w: %u, queue(%u/%u)",
		msg_queue->dsp_id,
		msg_queue->idx_r,
		msg_queue->idx_w,
	...
	uint32_t *p_queue_counter)
{
	...
	if (msg_queue == NULL || buf == NULL ||
		...
		pr_debug("NULL!! msg_queue: %p, buf: %p, p_idx_msg: %p, p_queue_counter: %p",
	...
	uint32_t *p_idx_msg)
{
	if (msg_queue == NULL || pp_scp_msg == NULL || p_idx_msg == NULL) {
		pr_debug("NULL!! msg_queue: %p, pp_scp_msg: %p, p_idx_msg: %p",
	...
	if (is_empty == true) {
		for (try_cnt = 0; try_cnt < k_max_try_cnt; try_cnt++) {
			...
			if (retval == 0) { /* got msg in queue */
				pr_debug("wait ret 0, empty %d, enable %d",
					  scp_check_queue_empty(msg_queue),
					  msg_queue->enable);
				break;
			}
	...
	const uint32_t scp_path)
{
	...
	if (dsp_id >= NUM_OPENDSP_TYPE || scp_path >= SCP_NUM_PATH) {
		pr_debug("dsp_id: %u, scp_path: %u error!!",
			dsp_id, scp_path);
		return -EFAULT;
	}
...
static int scp_process_msg_thread(void *data)
{
	...
	while (msg_queue->thread_enable && !kthread_should_stop()) {
		...
		if (retval != 0) {
			pr_debug("scp_get_queue_element retval %d", retval);
			continue;
		}
		if (idx_msg >= MAX_SCP_MSG_NUM_IN_QUEUE) {
			pr_debug("scp_get_queue_element idx_msg %u > %u",
	...
	pr_debug("thread exit, dsp_id %u, scp_path %u, idx_r %d, idx_w %d",
	...
	struct scp_msg_t *p_scp_msg)
{
	...
	if (msg_queue == NULL || p_scp_msg == NULL) {
		pr_debug("NULL!! msg_queue: %p, p_scp_msg: %p",
			msg_queue, p_scp_msg);
		return -EFAULT;
	}
	...
	case AUDIO_OPENDSP_USE_CM4_B:
		...
		retval = 0;
#else
		pr_debug("dsp_id %u not build!!", dsp_id);
		retval = -ENODEV;
#endif
		break;
	...
	struct scp_msg_t *p_scp_msg)
{
	...
	if (p_scp_msg->ipi_handler == NULL) {
		pr_debug("NULL!! p_scp_msg->ipi_handler: %p",
			p_scp_msg->ipi_handler);
		return -EFAULT;
	}

	if (p_scp_msg->len == 0) {
		pr_debug("p_scp_msg->len: %u", p_scp_msg->len);
		return -EFAULT;
	}
