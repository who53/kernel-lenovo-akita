...
char* boardid_get(void)
{
	...
	length = ((ptr-s1) > HARDWARE_MAX_ITEM_LONGTH) ? HARDWARE_MAX_ITEM_LONGTH :  (ptr-s1);
...
extern int sensorlist_set_hardware_info(int cmd);
int get_boardinfo(char *buf)
{
	int i = 0;
	char temp_buffer[HARDWARE_MAX_ITEM_LONGTH];
	int buf_size = 0;

	sensorlist_set_hardware_info(HARDWARE_ACCELEROMETER);
	sensorlist_set_hardware_info(HARDWARE_ALSPS);
	sensorlist_set_hardware_info(HARDWARE_GYROSCOPE);
	sensorlist_set_hardware_info(HARDWARE_MAGNETOMETER);
	...
	for(i = 0; i < HARDWARE_MAX_ITEM; i++)
	{
		...
		{
			sprintf(temp_buffer, "%s : %s\n", hardwareinfo_items[i], "MT6631");
		}
		else if (i == HARDWARE_FLASH || i == HARDWARE_BACK_SUB_CAM || i == HARDWARE_BACK_SUBCAM_MOUDULE_ID || i == HARDWARE_HARDWARE_ID)
			continue;
		else
		{
			sprintf(temp_buffer, "%s : %s\n", hardwareinfo_items[i], hardwareinfo_name[i]);
...
EXPORT_SYMBOL_GPL(get_boardinfo);
...
static struct platform_device boardinfo_device = {
	.name = PLATFORM_BOARDINFO_NAME,
	.dev = {}
};
...
static int __init hardwareinfo_init_module(void)
{
	...
	if (platform_device_register(&boardinfo_device)) {
		pr_err("failed to register boardinfo device\n");
		return -ENODEV;
	}
	ret = platform_driver_register(&boardinfo_driver);
	if(ret != 0)
	{
		pr_err("failed to register boardinfo driver\n");
		return -ENODEV;
	}
