...
static int otg_iddig_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(pinctrl)) {
		dev_dbg(&pdev->dev, "No find id pinctrl!\n");
		return -1;
	}
	...
	if (IS_ERR(info->id_init))
		dev_dbg(&pdev->dev, "No find pinctrl id_init\n");
	else
	...
	if (IS_ERR(info->id_enable))
		dev_dbg(&pdev->dev, "No find pinctrl iddig_enable\n");
	if (IS_ERR(info->id_disable))
		dev_dbg(&pdev->dev, "No find pinctrl iddig_disable\n");
	...
	ret = devm_request_irq(dev, info->id_irq, iddig_eint_isr,
					0, pdev->name, info);
	if (ret < 0) {
		dev_dbg(dev, "failed to request handler for ID IRQ\n");
		return ret;
	}
