...
static int i2c_get_semaphore(struct mt_i2c *i2c)
{
	...
	if (i2c->appm) {
		if (cpuhvfs_get_dvfsp_semaphore(SEMA_I2C_DRV) != 0) {
			dev_dbg(i2c->dev, "sema time out 2ms\n");
			if (cpuhvfs_get_dvfsp_semaphore(SEMA_I2C_DRV) != 0) {
				dev_dbg(i2c->dev, "sema time out 4ms\n");
				i2c_dump_info(i2c);
				WARN_ON(1);
				return -EBUSY;
			}
...
void i2c_dump_info(struct mt_i2c *i2c)
{
	/* I2CFUC(); */
	/* int val=0; */
	pr_debug_ratelimited("i2c_dump_info +++++++++++++++++++\n");
	...
	struct i2c_msg msgs[], int num)
{
	...
	while (left_num--) {
		...
		if (i2c_get_semaphore(i2c) != 0) {
			dev_dbg(i2c->dev, "get hw semaphore failed.\n");
			return -EBUSY;
		}
		...
		if (i2c_release_semaphore(i2c) != 0) {
			dev_dbg(i2c->dev, "release hw semaphore failed.\n");
			ret = -EBUSY;
		}
		if (ret < 0)
			goto err_exit;
	...
	if (!i2c->is_hw_trig) {
		...
		if (!i2c->irq_stat) {
			...
			if ((i2c->irq_stat & (I2C_IBI | I2C_BUS_ERR))) {
				...
				dev_dbg(i2c->dev, "0x84=0x%x\n",
					i2c_readw(i2c, OFFSET_ERROR));

				pr_debug("[bxx]0xE0=0x%x,0x1E0=0x%x,0x2E0=0x%x\n",
					_i2c_readw(i2c, 0xE0),
					_i2c_readw(i2c, 0x1E0),
					_i2c_readw(i2c, 0x2E0));
				pr_debug("[bxx]0xE4=0x%x,0x1E4=0x%x,0x2E4=0x%x\n",
					_i2c_readw(i2c, 0xE4),
					_i2c_readw(i2c, 0x1E4),
					_i2c_readw(i2c, 0x2E4));
				pr_debug("[bxx]0xE8=0x%x,0x1E8=0x%x,0x2E8=0x%x\n",
					_i2c_readw(i2c, 0xE8),
					_i2c_readw(i2c, 0x1E8),
					_i2c_readw(i2c, 0x2E8));
				pr_debug("[bxx]0xEC=0x%x,0x1EC=0x%x,0x2EC=0x%x\n",
					_i2c_readw(i2c, 0xEC),
					_i2c_readw(i2c, 0x1EC),
					_i2c_readw(i2c, 0x2EC));
				pr_debug("[bxx]0x58=0x%x,0x158=0x%x,0x258=0x%x\n",
					_i2c_readw(i2c, 0x58),
					_i2c_readw(i2c, 0x158),
					_i2c_readw(i2c, 0x258));
				/* IBI triggered */
				pr_debug("[bxx]0x54=0x%x,0x154=0x%x,0x254=0x%x\n",
					_i2c_readw(i2c, 0x54),
...
static int mt_i2c_probe(struct platform_device *pdev)
{
	...
	if (i2c->gpio_start == 0) {
		...
		if (IS_ERR(i2c->gpiobase)) {
			i2c->gpiobase = NULL;
			dev_dbg(&pdev->dev, "gpio baseaddress remap fail\n");
		}
	}
