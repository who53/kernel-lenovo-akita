...
void msdc_clr_fifo(unsigned int id)
{
	...
	if (MSDC_READ32(MSDC_DMA_CFG) & MSDC_DMA_CFG_STS) {
		...
		msdc_retry((MSDC_READ32(MSDC_DMA_CFG) & MSDC_DMA_CFG_STS),
			retry, cnt, id);
		if (retry == 0) {
			pr_debug("<<<WARN>>>: msdc%d, faield to stop DMA before clear FIFO, MSDC_DMA_CFG=0x%x\n",
				id, MSDC_READ32(MSDC_DMA_CFG));
			return;
		}
	}
...
static void msdc_pm(pm_message_t state, void *data)
{
	...
	if (evt == PM_EVENT_SUSPEND || evt == PM_EVENT_USER_SUSPEND) {
		...
		pr_debug("sdio: msdc_pm_resume host->suspend = %d,host->power_mode = %d",
	...
	if ((evt == PM_EVENT_SUSPEND) || (evt == PM_EVENT_USER_SUSPEND)) {
		if ((host->hw->host_function == MSDC_SDIO) &&
			...
			pr_debug("msdc%d -> MSDC Device Request Suspend",
				host->id);
		}
		msdc_clk_disable(host);
		spin_lock_irqsave(&msdc_cg_lock, flags);
		pr_debug("sdio:  suspend msdc_cg_cnt = %d", msdc_cg_cnt);
		msdc_cg_cnt--;
		if (msdc_cg_cnt == 0)
		...
		if ((evt == PM_EVENT_RESUME) || (evt == PM_EVENT_USER_RESUME)) {
			...
			if ((host->hw->host_function == MSDC_SDIO) &&
				...
				pr_debug("msdc%d -> MSDC Device Request Resume",
					host->id);
			}
			msdc_clk_enable_and_stable(host);
			spin_lock_irqsave(&msdc_cg_lock, flags);
			pr_debug("sdio: resume msdc_cg_cnt = %d", msdc_cg_cnt);
			msdc_cg_cnt++;
			if (msdc_cg_cnt == 1)
...
static void msdc_ops_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (mrq->data) {
		...
		if (cq_host && cq_host->enabled) {
			pr_debug("WARN: data xf with cqhci enabled\n");
			cq_host_en = true;
...
static int msdc_cqhci_pre_irq_complete(struct mmc_host *mmc, unsigned int err)
{
	...
	if (!err && (msdc_int & msdc_int_err)) {
		...
		pr_debug("%s %d: msdc_int = 0x%x, err = %d\n",
...
static int __init mt_msdc_init(void)
{
	...
	if (ret) {
		pr_debug(DRV_NAME2 ": Can't register driver");
		return ret;
	}
#endif
