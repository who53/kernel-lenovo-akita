...
static void pogo_work(struct work_struct *work)
{
	...
	if (!mt_usb_is_device() && (vbus > THUB_BASE_VBUS)) {
		pogo_enable_otg(1);
		pogo_enable_wrx(1);
	}
	...
	if (info->irq == info->irq_pogo0) {
		dev_dbg(info->dev, "%s: intr gpio %d value = %d vbus = %d\n",
		...
		dev_dbg(info->dev, "%s: intr gpio %d value = %d\n",
