...
void fgauge_get_profile_id(void)
{
	...
	if (ret != 0)
		bm_print(BM_LOG_CRTI,
				"[fgauge_get_profile_id]id_volt read fail\n");
	else
		bm_print(BM_LOG_CRTI,
				"[fgauge_get_profile_id]id_volt = %d\n", id_volt);

	if ((sizeof(g_battery_id_voltage) / sizeof(signed int)) != TOTAL_BATTERY_NUMBER) {
		bm_print(BM_LOG_CRTI,
			"[fgauge_get_profile_id]error! voltage range incorrect!\n");
		return;
	}
	...
	bm_print(BM_LOG_CRTI,
		"[fgauge_get_profile_id]Battery id (%d)\n", g_fg_battery_id);
}
				...
				struct battery_profile_struct *profile_p)
{
	...
	while (!of_property_read_u32_index(np, node_srting, idx, &addr)) {
		...
		msleep(20);
		bm_print(BM_LOG_CRTI,
			"__batt_meter_parse_table>> %s[%d]: <%d, %d>\n",
...
signed int fgauge_get_Q_max_high_current(signed short temperature)
{
	...
	bm_print(BM_LOG_FULL,
		"[fgauge_get_Q_max_high_current] Q_max = %d\r\n", ret_Q_max);
...
void sw_oam_init_v2(void)
{
	...
	ret = battery_meter_ctrl(
			BATTERY_METER_CMD_GET_BATTERY_PLUG_STATUS, &plugout_status);
...
signed int battery_meter_initial(void)
{
	...
	if (meter_initilized == false) {
		...
		table_init();
		bm_print(BM_LOG_CRTI,
				"[battery_meter_initial] SOC_BY_AUXADC done\n");
#endif
		...
		bm_print(BM_LOG_CRTI,
				"[battery_meter_initial] SOC_BY_HW_FG done\n");
#endif
		...
		oam_init();
		bm_print(BM_LOG_CRTI,
				"[battery_meter_initial] SOC_BY_SW_FG done\n");
#endif
...
#if defined(SOC_BY_HW_FG)
	bm_print(BM_LOG_CRTI,
		"[FGADC] Update DOD0(%d) by %d \r\n", gFG_DOD0, gFG_DOD1);
	gFG_DOD0 = gFG_DOD1;
#endif

#if defined(SOC_BY_SW_FG)
	bm_print(BM_LOG_CRTI,
		"[FGADC] Update oam_d0(%d) by %d \r\n", oam_d0, oam_d_5);
	oam_d0 = oam_d_5;
	gFG_DOD0 = oam_d0;
	oam_d_1 = oam_d_5;
	oam_d_2 = oam_d_5;
...
#if defined(SOC_BY_HW_FG)
	bm_print(BM_LOG_CRTI,
			"[battery_meter_reset]1 DOD0=%d,DOD1=%d,ui=%d\n",
			gFG_DOD0, gFG_DOD1, ui_percentage);
	gFG_DOD0 = 100 - ui_percentage;
	gFG_DOD1 = gFG_DOD0;
	bm_print(BM_LOG_CRTI,
			"[battery_meter_reset]2 DOD0=%d,DOD1=%d,ui=%d\n",
			gFG_DOD0, gFG_DOD1, ui_percentage);
#endif
...
int init_proc_log_fg(void)
{
	...
	if (proc_entry_fgadc == NULL) {
		ret = -ENOMEM;
		bm_print(BM_LOG_CRTI,
			"init_proc_log_fg: Couldn't create proc entry\n");
	} else {
		...
		struct device_attribute *attr, char *buf)
{
	...
	bm_print(BM_LOG_CRTI,
			"[FG] gFG_current_inout_battery : %d\n",
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_Current);
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_g_fg_dbg_bat_volt);
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_g_fg_dbg_bat_current);
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_g_fg_dbg_bat_zcv);
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_g_fg_dbg_bat_temp);
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_g_fg_dbg_bat_r);
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_g_fg_dbg_bat_car);
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_g_fg_dbg_bat_qmax);
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_g_fg_dbg_d0);
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_g_fg_dbg_d1);
	ret_device_file = device_create_file(
				&(dev->dev), &dev_attr_FG_g_fg_dbg_percentage);
	ret_device_file = device_create_file(
	...
	ret_device_file = device_create_file(
			&(dev->dev), &dev_attr_FG_Min_Battery_Voltage);
	ret_device_file = device_create_file(
			&(dev->dev), &dev_attr_FG_Max_Battery_Current);
	ret_device_file = device_create_file(
			&(dev->dev), &dev_attr_FG_Min_Battery_Current);
	ret_device_file = device_create_file(
...
void battery_aging_check(void)
{
	...
	/* gauge correct */
	battery_meter_ctrl(
			BATTERY_METER_CMD_GET_HW_FG_CAR, &columb_after_sleep);
...
static int battery_meter_resume(struct platform_device *dev)
{
	...
	if (now_time.tv_sec - last_oam_run_time.tv_sec > 30) {
		bat_spm_timeout = true;
		pr_debug("[battery_meter] trigger oam_run() for 30s threshold.\n");
	}
