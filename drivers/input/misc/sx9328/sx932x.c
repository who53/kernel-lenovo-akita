...
static int manual_offset_calibration(psx93XX_t this)
{
    ...
    msleep(50);
    returnValue += write_register(this,SX932x_CTRL1_REG,0x27);
    msleep(50);
	return returnValue;
}
...
#ifdef SAR_APK
struct raw_data_t raw_data_phase[4];
#endif
static void read_rawData(psx93XX_t this)
{
	...
	if(this){
		for(csx =0; csx<4; csx++){
...
#else
			raw_data_phase[csx].useful = (s32)((msb << 8) | lsb);
#endif
...
#else
			raw_data_phase[csx].average = (s32)((msb << 8) | lsb);
#endif			
            ...
            diff = (s32)((msb << 8) | lsb);
#else
			raw_data_phase[csx].diff = (s32)((msb << 8) | lsb);
#endif			
            ...
            if (diff > 32767)
               diff -= 65536;
            dev_info(this->pdev, " [CS: %d] Useful2 = %d Average = %d, DIFF = %d Offset = %d \n",csx,useful,average,diff,offset);
#else
			raw_data_phase[csx].offset = (u16)((msb << 8) | lsb);
			if (raw_data_phase[csx].useful > 32767)
				raw_data_phase[csx].useful -= 65536;
			if (raw_data_phase[csx].average > 32767)
				raw_data_phase[csx].average -= 65536;
			if (raw_data_phase[csx].diff > 32767)
				raw_data_phase[csx].diff -= 65536;
			dev_info(this->pdev, " [CS: %d] Useful = %d Average = %d, DIFF = %d Offset = %d \n",csx,raw_data_phase[csx].useful,raw_data_phase[csx].average,raw_data_phase[csx].diff,raw_data_phase[csx].offset);
#endif
		}
	}
}
						...
						struct device_attribute *attr, char *buf)
{
	...
	ret = sprintf(buf, "0,%d,%d,%d,%d,1,%d,%d,%d,%d,2,%d,%d,%d,%d,3,%d,%d,%d,%d \n",
                        ...
                        //"2,%d,%d,%d,%d,"
                        //"3,%d,%d,%d,%d \n",
                        raw_data_phase[0].useful,
                        raw_data_phase[0].average,
                        raw_data_phase[0].diff,
                        raw_data_phase[0].offset,

                       	raw_data_phase[1].useful,
                        raw_data_phase[1].average,
                        raw_data_phase[1].diff,
                        raw_data_phase[1].offset,

                        raw_data_phase[2].useful,
                        raw_data_phase[2].average,
                        raw_data_phase[2].diff,
                        raw_data_phase[2].offset,

                        raw_data_phase[3].useful,
                        raw_data_phase[3].average,
                        raw_data_phase[3].diff,
                        raw_data_phase[3].offset
                   );
	return ret;
#else
    return 0;
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(enable == 0)
	{
		...
		/* when disabled report a out key */
		/*if (this && (pDevice = this->pDevice))
		{
			input = pDevice->pbuttonInformation->input;
			input_report_key(input, 253, 1);
			input_report_key(input, 254, 0);
			input_sync(input);
		}*/
	}else{
		...
		const char *buf, size_t count)
{
	int ret = 0;
        ...
        printk("sx932x write_enable_store end ,val = %d\n",val);
   	return count;
}

static CLASS_ATTR(enable, 0664, /*read_enable_show*/NULL, write_enable_store);
...
{    
	...
	dev_info(&client->dev, "sx932x_probe() start\n");
    ...
    #ifdef SAR_WAKEUP
	wakeup_source_init(&wakesrc, "sx932x");
    #endif
	if (this){
...
{
#ifdef _CONFIG_OF
	printk("sx932x_i2c_init 4\n");
#endif
	if( i2c_add_driver(&sx932x_driver)){
        printk("sx932x_i2c_init i2c_add_driver error\n");
        return -1;
    }
    return 0 ;
}
