...
static int manual_offset_calibration(psx93XX_t this)
{
	s32 returnValue = 0;
        ...
        reg_val &= 0xF0;
        returnValue |= write_register(this,sx9310_CPS_CTRL0_REG,reg_val);
        msleep(50);
			   ...
			   struct device_attribute *attr, const char *buf, size_t count)
{
	...
	CAP_ERR("%s - Register(0x%2x) data(0x%2x)\n",__func__, regist, val);
...
#if 1
    dNode = of_find_compatible_node(NULL, NULL, "mediatek,SAR");
/*
...
*/
    if(dNode){
        this->irq_num = of_get_named_gpio(dNode, "int-gpio", 0);
        ...
        gpio_direction_input(this->irq_num);
		...
		const char *buf, size_t count)
{
	int ret = 0;
        ...
        CAP_DBG("write_enable_store end\n");
   	return count;
}
/*
...
static int sx9311_check_id(struct i2c_client *client)
{
    u8 value = 0;
    value = i2c_smbus_read_byte_data(client,sx9310_WHOAMI_REG);
    if(value == 0x02){
        return 1;
    } else {
        return -1;
	...
	if (this){
		...
		* (1->NIRQ=0, 0->NIRQ=1) */
		...
		if (MAX_NUM_STATUS_BITS>= 8)
		{
			...
			this->statusFunc[4] = touchProcess;//0;//read_regStat; /* COMP_STAT */
...
#if USE_SENSOR_INVALID_DETECT
void sx93XX_invalid_func(struct work_struct *work)
{
    psx93XX_t this = psx93xx;
    psx9310_t pDevice = NULL;
    struct input_dev *input = NULL;
    ...
    read_register(this,sx9310_WHOAMI_REG,&value);
    if(value != 0x02)
        invalid |= 1;
    read_register(this,sx9310_CPS_CTRL18_REG,&value);
    if(value != 0x25)
        invalid |= 2;
    read_register(this,sx9310_CPS_CTRL8_REG,&value);
    if(value != 0x28)
        invalid |= 4;
    read_register(this,sx9310_CPS_CTRL9_REG,&value);
    if(value != 0x38)
        invalid |= 8;
    if(invalid != 0)
    {
        input_report_key(input, buttons[1].keycode, 1);
        input_sync(input);
        input_report_key(input, buttons[2].keycode, 1);
        input_sync(input);
...
#if USE_SENSOR_INVALID_DETECT
    INIT_DELAYED_WORK(&this->invalid_worker, sx93XX_invalid_func);
#endif
