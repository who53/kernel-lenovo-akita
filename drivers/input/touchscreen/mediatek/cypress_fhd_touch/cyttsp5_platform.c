...
struct cyttsp5_loader_platform_data _cyttsp5_loader_platform_data = {
	...
	.flags = CY_LOADER_FLAG_CALIBRATE_AFTER_FW_UPGRADE,//CY_LOADER_FLAG_NONE
};

//samir add
struct pinctrl *power_pins;
struct pinctrl_state *powerpins_default, *powerpins_output0, *powerpins_output1;

int cyttsp5_get_powerpins_info(struct device *dev)
{
    int ret = 0;
    
    printk(KERN_ERR "%s [cyttsp5] get powerpins info ..\n",__func__);
    ...
    if (IS_ERR(powerpins_default)) {
	    ret = PTR_ERR(powerpins_default);
	    printk(KERN_ERR "%s Cannot find power_pins default %d!\n",__func__, ret);
    }
    powerpins_output0 =	pinctrl_lookup_state(power_pins, "ctp_power_off");
    if (IS_ERR(powerpins_output0)) {
	    ret = PTR_ERR(powerpins_output0);
	    printk(KERN_ERR "%s Cannot find power_pins ctp_power_off!\n",__func__);
	    return ret;
    }
    powerpins_output1 =	pinctrl_lookup_state(power_pins, "ctp_power_on");
    if (IS_ERR(powerpins_output1)) {
	    ret = PTR_ERR(powerpins_output1);
	    printk(KERN_ERR "%s Cannot find power_pins ctp_power_on!\n",__func__);
	    return ret;
    }
    return 0;
}

void cyttsp5_powerpins_output(int level)
{
    printk(KERN_ERR"%s [cyttsp5] power enable = %d \n", __func__, level);

    if (level)
        pinctrl_select_state(power_pins, powerpins_output1);
    else
        pinctrl_select_state(power_pins, powerpins_output0);
} 
//samir end
		...
		struct device *dev)
{
	...
	//tpd_gpio_output(0,1);// reset pin high
        cyttsp5_powerpins_output(1);
	// Here to fill 3.3V on 1.8V on
	msleep(DELAY_BOOT_READY);

	// Here to fill 3.3V off 1.8V off
	//tpd_gpio_output(0,0);// reset pin low
        cyttsp5_powerpins_output(0);
...
{
#ifdef CONFIG_YUDE_MTK
                cyttsp5_get_powerpins_info(dev);
