...
static struct cyttsp5_core_platform_data _cyttsp5_core_platform_data = {
	...
	.rst_gpio = 0,
	.hid_desc_register = 1,
	.xres = cyttsp5_xres, // Assign NULL to it if no TP reset control
	.init = cyttsp5_init,
	.power = cyttsp5_power,
	...
	.flags = CY_CORE_FLAG_POWEROFF_ON_SLEEP, /* CY_CORE_FLAG_RESTORE_PARAMETERS */
	.easy_wakeup_gesture = CY_CORE_EWG_NONE,
	.level_irq_udelay = 1, /* 1- level trigger 0-edge trigger */
};
...
static const struct of_device_id mtk_dt_match_table[] = {
	{.compatible = "mediatek,cypress_touch"},
	{},
};
...
//#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_DEVICETREE_SUPPORT
	...
	if (!match) {
		dev_err(dev, "%s: No device match found\n", __func__);
	}
