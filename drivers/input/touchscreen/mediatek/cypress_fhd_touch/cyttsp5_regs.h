...
#define DELAY_BOOT_READY	40
#define DELAY_RESET_LOW		20
#define DELAY_UI_READY		20
#endif

#ifdef USE_I2C_DMA
#undef  FTS_DMA_BUF_SIZE
...
#define CY_HID_GET_REPORT_DESCRIPTOR_TIMEOUT	1000
#define CY_HID_SET_POWER_TIMEOUT		1000
#ifdef VERBOSE_DEBUG
...
#define CY_HID_OUTPUT_RESUME_SCAN_TIMEOUT	2000
#define CY_HID_OUTPUT_CALIBRATE_IDAC_TIMEOUT	5000
#define CY_HID_OUTPUT_WRITE_CONF_BLOCK_TIMEOUT	1000
#define CY_HID_OUTPUT_RUN_SELF_TEST_TIMEOUT	10000
#define CY_HID_OUTPUT_BL_INITIATE_BL_TIMEOUT	20000
#define CY_HID_OUTPUT_BL_PROGRAM_AND_VERIFY_TIMEOUT	1000
		...
		void *buf, int size)
{
	return cd->bus_ops->read_default(cd->dev, buf, size,
			cd->I2CDMABuf_va, cd->I2CDMABuf_pa);
}
		...
		u8 write_len, u8 *write_buf, u8 *read_buf)
{
	return cd->bus_ops->write_read_specific(cd->dev, write_len, write_buf,
			read_buf, cd->I2CDMABuf_va, cd->I2CDMABuf_pa);
}
