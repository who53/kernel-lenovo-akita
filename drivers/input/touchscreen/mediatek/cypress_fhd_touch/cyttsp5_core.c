...
int cyttsp5_hid_output_get_sysinfo(struct cyttsp5_core_data *cd)
{
	int rc;
...
static int cyttsp5_hid_output_resume_scanning_(struct cyttsp5_core_data *cd)
{
	struct cyttsp5_hid_output hid_output = {
		...
		.timeout_ms = CY_HID_OUTPUT_RESUME_SCAN_TIMEOUT,
	};
...
static int cyttsp5_core_sleep_(struct cyttsp5_core_data *cd)
{
	...
	if ((cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP) &&
		IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture))
...
{
	struct cyttsp5_core_data *cd =g_cd;
	...
	if (device_may_wakeup(cd->dev)) {
		dev_vdbg(cd->dev, "%s Device MAY wakeup\n", __func__);
		if (!enable_irq_wake(cd->irq))
			cd->irq_wake = 1;
	} else {
		dev_dbg(cd->dev, "%s Device MAY NOT wakeup\n", __func__);
	}

	return 0;
}
EXPORT_SYMBOL_GPL(cyttsp5_core_suspend);
...
EXPORT_SYMBOL_GPL(cyttsp5_core_resume);
...
void cyttsp5_setup_fb_notifier(struct cyttsp5_core_data *cd)
{
	int rc;
...
/*samir add 20190708*/
extern int cyttsp5_cmcp_test(void);
...
static ssize_t ctp_open_proc_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
    int result = 0;
    int len = count;
    if(*ppos){
    ...
    *ppos += count;
    /* samir add 20190709 */
    result = cyttsp5_cmcp_test();
    
    if(count > 9){
        len = 9;
    }
    
    printk("cyttsp5 ts result = %d\n", result);
    if(result == 0){
...
#ifdef USE_I2C_DMA
	cd->I2CDMABuf_va = (u8 *)dma_alloc_coherent(dev, FTS_DMA_BUF_SIZE,
			(dma_addr_t *)&cd->I2CDMABuf_pa, GFP_KERNEL);
	if (!cd->I2CDMABuf_va) {
...
error_detect:
	...
	kfree(cd);
#ifdef USE_I2C_DMA
error_alloc_dma_data:
	dma_free_coherent(dev, FTS_DMA_BUF_SIZE, cd->I2CDMABuf_va,
			cd->I2CDMABuf_pa);
#endif
error_alloc_data:
error_no_pdata:
...
#ifdef USE_I2C_DMA
		if(cd->I2CDMABuf_va){
			dma_free_coherent(dev, FTS_DMA_BUF_SIZE, cd->I2CDMABuf_va, cd->I2CDMABuf_pa);
		}
#endif
